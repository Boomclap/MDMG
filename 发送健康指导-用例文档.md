...
6. **建议内容实时同步到编辑器**
   - **选中建议时的同步机制**：
     ...
     * 系统调用 `updateEditorWithSuggestions()` 函数，扫描所有已选中的建议项
     * 根据建议类型生成相应的文本格式：
       - 文本建议：直接显示建议内容
   ...
   - **编辑器内容结构化生成**：
     ...
     * 将所有选中建议以无序列表（`<ul><li>`）形式组织
     * 每个建议项作为一个列表项，支持富文本格式（加粗标题）
     * 保持固定的结束语："请继续保持良好的自我管理，如有任何不适，请及时联系我。"
   
   - **实时预览效果**：
     * 建议选择状态变化时，编辑器内容立即更新，无需手动刷新
     * 数值参数调整时，编辑器中对应的数值实时变化
     * 单选建议切换时，编辑器中自动替换为新选中的内容
     * 多选建议的增减会动态添加或移除对应的列表项
...
### 6b. 编辑器内容冲突
6b1. 如果用户手动编辑了编辑器内容，再选择建议时提示"是否覆盖当前内容"
6b2. 提供"合并内容"和"替换内容"两个选项
6b3. 合并模式下，新建议追加到现有内容后
6b4. 替换模式下，完全重新生成编辑器内容
...
function updateEditorWithSuggestions() {
    ...
    template.suggestions.text.forEach(suggestion => {
        const element = document.querySelector(`[onclick*="${suggestion.id}"]`);
    ...
    Object.keys(template.suggestions.multiple).forEach(groupKey => {
        ...
        group.options.forEach(option => {
            const element = document.querySelector(`[onclick*="${option.id}"]`);
            const checkbox = element?.querySelector('.suggestion-checkbox');
            if (checkbox?.classList.contains('checked')) {
                selectedSuggestions.push({
                    type: 'multiple',
    ...
    Object.keys(template.suggestions.single).forEach(groupKey => {
        ...
        group.options.forEach(option => {
            const element = document.querySelector(`[onclick*="${option.id}"]`);
            const radio = element?.querySelector('.suggestion-radio');
    ...
    Object.keys(template.suggestions.numeric || {}).forEach(numericKey => {
        ...
        const element = document.querySelector(`[onclick*="toggleNumericSuggestion(this, '${numericKey}')"]`);
        const checkbox = element?.querySelector('.suggestion-checkbox');
        if (checkbox?.classList.contains('checked')) {
            const input = document.getElementById(`numeric-${numericKey}`);
            const value = input ? parseInt(input.value) : numeric.defaultValue;
            selectedSuggestions.push({
                type: 'numeric',
                title: numeric.title,
                content: `${value}${numeric.unit}`
            });
        }
    });
    ...
    if (selectedSuggestions.length > 0) {
        ...
        selectedSuggestions.forEach(suggestion => {
            if (suggestion.title && suggestion.title !== suggestion.content) {
...
```javascript
// 每个建议项点击时都会触发对应的处理函数
// 处理函数最后都会调用updateEditorWithSuggestions()

// 文本建议点击处理
function toggleTextSuggestion(element, suggestionId) {
    const checkbox = element.querySelector('.suggestion-checkbox');
    checkbox.classList.toggle('checked');
    updateEditorWithSuggestions(); // 立即同步
}
